...
//   // Filter dishes based on category and search term
...
//                       {dish.calories_value} kcal | Protein: {dish.protein_value}g | Fat: {dish.fat_value}g | Carbs: {dish.carbs_value}g
//                     </p>
//                     {dish.health_condition?.length > 0 && (
...
function AddDish({ goBack, refreshDishes, editingDish = null }) {
  ...
  async function handleImageChange(index, e) {
    ...
    try {
      ...
      setNewDishes((prev) => {
        const copy = [...prev];
        copy[index].image_url = publicData.publicUrl;
        return copy;
      });
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-white shadow-lg rounded-2xl p-6 border border-green-100">
      ...
      {newDishes.map((dish, idx) => (
        <div key={idx} className="mb-6 p-4 border rounded-lg bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="md:col-span-2 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    value={dish.steps}
                    onChange={(e) => handleChange(idx, "steps", e.target.value)}
                    rows={4}
                    placeholder="Step 1&#10;Step 2&#10;Step 3"
                    className="mt-1 w-full border border-green-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-400 transition resize-none"
                  />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 bg-white p-4 rounded-xl shadow-sm border border-green-100">
            {[
              { field: "calories_value", label: "Calories" },
              { field: "protein_value", label: "Protein" },
              { field: "fat_value", label: "Fat" },
              { field: "carbs_value", label: "Carbs" },
            ].map(({ field, label }) => (
              <div key={field}>
            ...
            {/* Additional Inputs */}
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              {["ingredient", "health_condition", "store", "dietary", "allergen", "goal"].map((field) => (
                <div key={field}>
                  <label className="text-sm font-semibold text-green-700">
...
export default function DishesAdmin() {
  ...
  // Delete multiple selected dishes
  async function deleteMultipleDishes() {
    if (selectedDishes.length === 0) return alert("No dishes selected.");
    if (!confirm(`Delete ${selectedDishes.length} dish(es)?`)) return;
    for (const dish of selectedDishes) {
      await deleteDish(dish.id, dish.image_url);
    }
    setSelectedDishes([]);
      ...
      {!showAddForm ? (
          ...
          <div className="bg-white shadow-md rounded-2xl p-6 mb-6 flex flex-wrap items-center justify-between gap-4 border border-green-100">
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              ...
              <div className="relative w-full sm:w-48">
                ...
                <FiChevronDown className="absolute right-3 top-1/2 transform -translate-y-1/2 text-green-600 pointer-events-none" />
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 justify-end w-full md:w-auto">
              <button
                onClick={() => {
                  setShowAddForm(true);
                  setEditingDish(null);
                  setSelectedDishes([]); // ✅ clear selections when adding
                }}
                className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold px-5 py-2.5 rounded-xl shadow-sm hover:shadow-md transition-transform transform hover:-translate-y-0.5"
              >
                <FiPlusCircle size={18} /> Add Dish
              </button>
              ...
              {selectedDishes.length > 0 && (
                <>
                  <button
                    ...
                    className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white font-semibold px-5 py-2.5 rounded-xl shadow-sm hover:shadow-md transition-transform transform hover:-translate-y-0.5"
                  >
                    <FiTrash2 size={18} /> Delete Selected (
                      ...
                      <div className="mt-3 flex gap-2">
                        <button
                          onClick={(e) => {
                            ...
                            setEditingDish(dish);
                            setShowAddForm(true);
                            setSelectedDishes([]); // ✅ Clear selected dishes when editing
                          }}
                          className="flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded"
                        >
                          <FiEdit /> Edit
        ...
        <AddDish
          goBack={() => {
            setShowAddForm(false);
            setEditingDish(null);
            setSelectedDishes([]); // ✅ Clear selections when returning from add/edit
            fetchDishes();
          }}
          refreshDishes={() => {
            fetchDishes();
            setSelectedDishes([]); // ✅ Clear selections after saving
          }}
          editingDish={editingDish}
        />
      )}
